...
public static class CLIHandler
{
    ...
    public static int WingetUIToUniGetUIMigrator()
    {
        try
        {
            ...
            {
                foreach (string old_wingetui_icon in new[] { "WingetUI.lnk", "WingetUI .lnk", "UniGetUI (formerly WingetUI) .lnk", "UniGetUI (formerly WingetUI).lnk", "UniGetUI.lnk", "UniGetUI .lnk" })
                {
                    try
                    {
                        string old_file = Path.Join(path, old_wingetui_icon);
                        string new_file = Path.Join(path, AppBranding.ShortcutName + ".lnk");
