...
namespace UniGetUI.Interface
{
    public abstract partial class AbstractPackagesPage : IKeyboardShortcutListener, IEnterLeaveListener, ISearchBoxPage
    {
        ...
        private bool _defaultSourcesSelected = false;
        ...
        public virtual bool IsPackageActionBlocked(IPackage package) => false;
        ...
        protected AbstractPackagesPage(PackagesPageData data)
        {
            ...
            ViewModeSelector.SelectedIndex = 2; // 強制使用圖示模式
        ...
        {
            // 檢查控制項是否已初始化，避免 NullReferenceException
            if (NameHeader is null || IdHeader is null || VersionHeader is null || 
                NewVersionHeader is null || SourceHeader is null)
            {
                return;
            }
        ...
        private void Loader_PackagesChanged(object? sender, PackagesChangedEvent packagesChangedEvent)
        {
            ...
            else
            {
                ...
                ClearSourcesList(); // This also resets _defaultSourcesSelected flag
        ...
        /// <summary>
        /// 透過 IPackage 取得對應的 PackageWrapper
        /// </summary>
        /// <param name="package">要查找的套件</param>
        /// <returns>對應的 PackageWrapper，如果找不到則返回 null</returns>
        protected PackageWrapper? GetPackageWrapper(IPackage package)
        {
            return WrappedPackages.FirstOrDefault(w => w.Package.Equals(package));
        }
        ...
        private void Loader_FinishedLoading(object? sender, EventArgs e)
        {
            ...
            // Auto-select aiDAPTIV-bucket when packages finish loading
            // Only do this once when packages first load
            if (!_defaultSourcesSelected)
            {
                ForceSelectOnlyAiDAPTIVBucket();
                _defaultSourcesSelected = true;
                // Re-filter packages to apply the source selection
                FilterPackages();
            }
        ...
        protected void AddPackageToSourcesList(IPackage package)
        {
            ...
            // Check if this source actually exists in the manager's configuration
            // This prevents showing sources that are not configured (e.g., aiDAPTIV-bucket not in Scoop settings)
            if (source.Manager.Capabilities.SupportsCustomSources)
            {
                var availableSources = source.Manager.SourcesHelper.Factory.GetAvailableSources();
                bool sourceExists = availableSources.Any(s =>
                    s.Name.Equals(source.Name, StringComparison.OrdinalIgnoreCase) ||
                    (s.Url != null && source.Url != null &&
                     s.Url.ToString().TrimEnd('/').Equals(source.Url.ToString().TrimEnd('/'), StringComparison.OrdinalIgnoreCase))
                );

                if (!sourceExists)
                {
                    // Source doesn't exist in actual configuration, skip adding it
                    Logger.Debug($"Skipping source '{source.Name}' as it's not found in {source.Manager.Properties.Name} configuration");
                    return;
                }
            }
            ...
            if (!UsedManagers.Contains(source.Manager))
            {
                ...
                // For Scoop, expand but don't select the parent node
                // We will only select specific child sources (aiDAPTIV-bucket)
                bool isScoop = source.Manager.Properties.Name == "Scoop";
                if (isScoop)
                {
                    node.IsExpanded = true; // Expand Scoop to show its sources
                    // Don't select the parent node - only child sources will be selected
                }
            ...
            if ((!UsedSourcesForManager.ContainsKey(source.Manager) || !UsedSourcesForManager[source.Manager].Contains(source)) && source.Manager.Capabilities.SupportsCustomSources)
            {
                ...
                else
                {
                    ...
                    // Note: Auto-selection of aiDAPTIV-bucket is handled in Loader_FinishedLoading
                    // to ensure all packages are loaded before selecting the source
                }
            }
        }

        /// <summary>
        /// Virtual method to allow derived classes to select default sources
        /// Called after packages are loaded
        /// </summary>
        protected virtual void SelectDefaultSources()
        {
            // Default implementation: do nothing, sources are already selected in AddPackageToSourcesList
            // Derived classes can override this for custom behavior
        }

        /// <summary>
        /// Forces the source selection to only aiDAPTIV-bucket if it exists.
        /// Clears all other selections and selects only aiDAPTIV-bucket.
        /// Should be called when entering a page to ensure default selection.
        /// </summary>
        protected void ForceSelectOnlyAiDAPTIVBucket()
        {
            try
            {
                ...
                // Find valid bucket source (from ValidBucketList.json)
                var aiDAPTIVBucketNode = NodesForSources.FirstOrDefault(kvp =>
                    kvp.Key.Manager.Properties.Name == "Scoop" &&
                    kvp.Key.Url != null &&
                    CoreData.IsValidBucket(kvp.Key.Url)
                );

                if (aiDAPTIVBucketNode.Value != null)
                {
                    // Select only aiDAPTIV-bucket
                    SourcesTreeView.SelectedNodes.Add(aiDAPTIVBucketNode.Value);

                    // Ensure the Scoop node is expanded so the user can see aiDAPTIV-bucket
                    var scoopManager = aiDAPTIVBucketNode.Key.Manager;
                    if (RootNodeForManager.TryGetValue(scoopManager, out var scoopNode))
                    {
                        scoopNode.IsExpanded = true;
                    }
                }
                // If aiDAPTIV-bucket is not found, leave selection empty (no packages will be shown)
            }
            catch (Exception ex)
            {
                Logger.Error($"Failed to select aiDAPTIV-bucket: {ex.Message}");
            }
        }
        ...
        protected void ClearSourcesList()
        {
            ...
            _defaultSourcesSelected = false; // Reset flag when clearing sources
        }
        ...
        protected void PerformMainPackageAction(IPackage? package)
        {
            ...
            if (IsPackageActionBlocked(package))
            {
                DialogHelper.ShowDismissableBalloon(
                    CoreTools.Translate("Invalid selection"),
                    CoreTools.Translate("This package is already installed"));
                return;
            }
        ...
        public virtual void OnEnter()
        {
            ...
            // Reset source selection to only aiDAPTIV-bucket when entering the page
            // This ensures that every time the user switches to this page, the default source is selected
            // We don't check _defaultSourcesSelected here because we want to force selection every time
            ForceSelectOnlyAiDAPTIVBucket();
        ...
        private void SetFilterMode_Inline()
        {
            ...
            SidePanel.Background = new SolidColorBrush(Microsoft.UI.Colors.Transparent);
