...
public static partial class DialogHelper
{
    ...
    /// 顯示啟動前置條件檢查失敗對話框
    /// </summary>
    /// <returns>使用者選擇的操作：true = 退出程式，false = 繼續（忽略）</returns>
    public static async Task<bool> ShowStartupRequirementsFailedAsync()
    {
        var failedRequirements = StartupRequirementsChecker.GetFailedRequirements().ToList();
        ...
        dialog.Title = CoreTools.Translate("Startup Requirements Not Met");
        dialog.PrimaryButtonText = CoreTools.Translate("Exit");
        dialog.SecondaryButtonText = CoreTools.Translate("Ignore and Continue");
        ...
        {
            Text = CoreTools.Translate("The following requirements are not met. The application may not work properly without them."),
        ...
        foreach (var requirement in failedRequirements)
        {
            ...
            if (!string.IsNullOrEmpty(requirement.ErrorMessage))
            {
                itemPanel.Children.Add(new TextBlock
                {
                    Text = $"  {requirement.ErrorMessage}",
                    TextWrapping = TextWrapping.Wrap,
                    Opacity = 0.8
                });
            }
            ...
            // 修復指令
            if (!string.IsNullOrEmpty(requirement.FixCommand))
            {
                itemPanel.Children.Add(new TextBlock
                {
                    Text = CoreTools.Translate("Fix command:"),
                    TextWrapping = TextWrapping.Wrap,
                    Opacity = 0.7,
        ...
        stackPanel.Children.Add(new TextBlock
        {
            Text = CoreTools.Translate("Please install the missing requirements and restart the application."),
            TextWrapping = TextWrapping.Wrap,
        ...
        return result == ContentDialogResult.Primary; // true = 退出程式
    }

    /// <summary>
    /// Show blocking dialog when aiDAPTIV startup fails
    /// </summary>
    public static async Task ShowAIDAPTIVStartupFailedAsync()
    {
        var dialog = DialogFactory.Create();
        dialog.Title = CoreTools.Translate("aiDAPTIV Startup Failed");
        ...
        {
            Text = CoreTools.Translate("Startup failed. wService_delete.bat, wService_create.bat, or aiDAPTIVService failed after 3 retries. The application will now close. Please verify permissions, required files, and related services, then start the application again."),
            TextWrapping = TextWrapping.Wrap
        };

        dialog.Closing += (_, e) =>
        {
            // Force explicit acknowledgement
    ...
    //     p.Inlines.Add(new LineBreak());
    //     p.Inlines.Add(new LineBreak());
    //     var link = new Hyperlink { NavigateUri = new Uri("https://github.com/aiDAPTIV-Phison/aiDAPTIVAppStore/wiki"), };
