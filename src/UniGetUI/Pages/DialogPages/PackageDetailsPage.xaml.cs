...
namespace UniGetUI.Interface.Dialogs
{
    ...
    {
        private static readonly Brush ReinstallBlockedButtonBackground = new SolidColorBrush(Color.FromArgb(64, 255, 193, 7));
        private static readonly Brush ReinstallBlockedButtonBorder = new SolidColorBrush(Color.FromArgb(255, 255, 193, 7));
        ...
        public PackageDetailsPage(IPackage package, OperationType role, TEL_InstallReferral referral)
        {
            ...
            if (OperationRole is OperationType.Install)
            {
                ...
                ApplyInstallBlockedHighlightIfNeeded();
            }
        ...
        private bool IsInstallActionBlocked(IPackage package)
            => package.Tag == PackageTag.AlreadyInstalled || package.GetInstalledPackages().Count > 0;

        private void ApplyInstallBlockedHighlightIfNeeded()
        {
            if (!IsInstallActionBlocked(Package))
            {
                return;
            }

            MainActionButton.Background = ReinstallBlockedButtonBackground;
            MainActionButton.BorderBrush = ReinstallBlockedButtonBorder;
            MainActionButton.BorderThickness = new Thickness(1);

            ExtendedActionButton.Background = ReinstallBlockedButtonBackground;
            ExtendedActionButton.BorderBrush = ReinstallBlockedButtonBorder;
            ExtendedActionButton.BorderThickness = new Thickness(1);
        }
        ...
        public async Task LoadInformation()
        {
            ...
            {
                SetTextToItem(InstallerSize_Content, details.InstallerSize > 0 ? $" ({CoreTools.FormatAsSize(details.InstallerSize, 2)})" : $" ({CoreTools.Translate("Unknown size")})");
                SetTextToItem(DownloadInstaller_Button, CoreTools.Translate("Download installer"));
            }
            else
            {
        ...
        {
            if (action is OperationType.Install && IsInstallActionBlocked(package))
            {
            ...
            var newOptions = await InstallOptionsPage.GetUpdatedOptions();
            await InstallOptionsFactory.SaveForPackageAsync(newOptions, package);
