...
public static class CrashHandler
{
    ...
    public static void ReportFatalException(Exception e)
    {
       ...
       Console.WriteLine(Error_String);
        ...
        string crashFile = Path.Join(
            CoreData.UniGetUIDataDirectory,
            $"crash-report-{DateTime.Now:yyyyMMdd-HHmmss}.txt");
        File.WriteAllText(crashFile, Error_String);
        Console.WriteLine($"Crash report saved to: {crashFile}");
        Environment.Exit(1);
    }
}
