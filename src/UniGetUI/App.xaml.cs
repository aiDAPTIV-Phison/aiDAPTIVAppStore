...
namespace UniGetUI
{
    public partial class MainApp
    {
        ...
        // aiDAPTIVService 管理器
        private AIDAPTIVServiceManager? _aidaptivServiceManager;
        ...
        private async Task LoadComponentsAsync()
        {
            try
            {
                ...
                // 先執行啟動前置條件檢查（包含自動新增 bucket）
                // 這必須在 LoadManagers 之前執行，確保 bucket 在 Scoop 讀取 sources 之前就已存在
                await Task.Run(CheckStartupRequirementsAsync);

                // 檢查啟動前置條件是否通過
                if (!StartupRequirementsChecker.AllPassed)
                {
                    Logger.Warn("Startup requirements check failed, showing dialog to user...");
                    bool shouldExit = await DialogHelper.ShowStartupRequirementsFailedAsync();
                    if (shouldExit)
                    {
                        Logger.ImportantInfo("User chose to exit due to failed startup requirements.");
                        DisposeAndQuit(1);
                        return;
                    }
                    Logger.Warn("User chose to continue despite failed startup requirements.");
                }

                // 現在 bucket 已經準備好，可以平行載入 managers 和其他元件
                ...
                // HIDDEN: Telemetry initialization
                // _ = TelemetryHandler.InitializeAsync();
                _ = IconDatabase.Instance.LoadIconAndScreenshotsDatabaseAsync();

                // 啟動 aiDAPTIVService（在所有元件載入完成後、切換主畫面之前）
                // 重試過程中不顯示 ErrorBanner，避免載入畫面出現「啟動失敗」字眼
                bool aiDaptivReady = await StartAIDAPTIVServiceWithRetryAsync();
                if (!aiDaptivReady)
                {
                    Logger.Error("aiDAPTIVService failed after retries. Blocking startup.");
                    await DialogHelper.ShowAIDAPTIVStartupFailedAsync();
                    DisposeAndQuit(1);
                    return;
                }
        ...
        /// <summary>
        /// 檢查啟動前置條件（Git、Scoop、versions bucket）
        /// 結果會存儲在 StartupRequirementsChecker.Results 中，
        /// 並在 LoadComponentsAsync 完成後進行處理
        /// </summary>
        private static async Task CheckStartupRequirementsAsync()
        {
            try
            {
                await StartupRequirementsChecker.CheckAllRequirementsAsync();
            }
            catch (Exception ex)
            {
                Logger.Error("Exception in CheckStartupRequirementsAsync:");
                Logger.Error(ex);
            }
        }
        ...
        public void DisposeAndQuit(int outputCode = 0)
        {
            ...
            StopValidBucketPackagesBeforeShutdown();

            // 停止 aiDAPTIVService
            try
            {
                if (_aidaptivServiceManager != null)
                {
                    Logger.Info("Stopping aiDAPTIVService...");
                    _aidaptivServiceManager.Stop();
                    Logger.Info("aiDAPTIVService cleanup completed");
                }
            }
            catch (Exception ex)
            {
                Logger.Error("Error stopping aiDAPTIVService:");
                Logger.Error(ex);
            }
        ...
        {
            Logger.Warn("KillAndRestart: Shutting down before restart");
            StopValidBucketPackagesBeforeShutdown();
            ...
            {
                Logger.Warn($"Failed to unregister AppInstance key: {ex.Message}");
            }
            ...
            try
            {
                if (_aidaptivServiceManager != null)
                {
                    Logger.Info("KillAndRestart: Stopping aiDAPTIVService...");
                    _aidaptivServiceManager.Stop();
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"KillAndRestart: Error stopping aiDAPTIVService: {ex.Message}");
            }
            ...
            Process.Start(new ProcessStartInfo(CoreData.UniGetUIExecutableFile)
            {
        ...
        private void StopValidBucketPackagesBeforeShutdown()
        {
            try
            {
                var (processedCount, failedCount) = Task.Run(ServiceManager.StopRunningValidBucketPackagesAsync).GetAwaiter().GetResult();
                Logger.Info($"Shutdown pre-stop completed. Processed={processedCount}, Failed={failedCount}");
            }
            catch (Exception ex)
            {
                Logger.Error($"Failed to stop ValidBucket packages before shutdown: {ex.Message}");
            }
        }

        private void ReportAIDAPTIVStartupStatus(string message)
        {
            Logger.Warn($"[aiDAPTIV Startup] {message}");
        }

        /// <summary>
        /// 啟動 aiDAPTIVService，若失敗重試 3 次
        /// </summary>
        private async Task<bool> StartAIDAPTIVServiceWithRetryAsync()
        {
            try
            {
                // Path to aiDAPTIVService.exe
                string servicePath = Path.Combine(
                    AppContext.BaseDirectory,
                    "appstore", "aiDAPTIV", "aiDAPTIVService.exe"
                );

                Logger.Info($"Attempting to start aiDAPTIVService from: {servicePath}");

                // Create manager (AIDAPTIVServiceManager will auto-detect config path)
                _aidaptivServiceManager = new AIDAPTIVServiceManager(servicePath);
                const int maxAttempts = 3;
                for (int attempt = 1; attempt <= maxAttempts; attempt++)
                {
                    ReportAIDAPTIVStartupStatus($"aiDAPTIVService startup attempt {attempt}/{maxAttempts}");
                    bool started = await Task.Run(() => _aidaptivServiceManager.Start(ReportAIDAPTIVStartupStatus));
                    if (started)
                    {
                        Logger.ImportantInfo("aiDAPTIVService started successfully");
                        return true;
                    }

                    if (attempt < maxAttempts)
                    {
                        ReportAIDAPTIVStartupStatus($"aiDAPTIVService startup failed, retrying attempt {attempt + 1}/{maxAttempts}");
                        await Task.Delay(1000);
                    }
                }

                Logger.Error("aiDAPTIVService failed after 3 start attempts");
                return false;
            }
            catch (Exception ex)
            {
                Logger.Error("Exception in StartAIDAPTIVService:");
                Logger.Error(ex);
                ReportAIDAPTIVStartupStatus($"aiDAPTIVService startup exception: {ex.Message}");
                return false;
            }
        }
    }
