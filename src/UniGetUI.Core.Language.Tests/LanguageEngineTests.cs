...
namespace UniGetUI.Core.Language.Tests
{
    public class LanguageEngineTests
    {
        ...
        [InlineData("ca", "Registre de UniGetUI", "UniGetUI (abans WingetUI)")]
        public void TestUniGetUIRefactoring(string language, string expectedLogTemplate, string expectedWingetUITemplate)
        {
            // Use CoreData.AppName which is set from AppBranding
            string appName = CoreData.AppName;
            string expectedLog = expectedLogTemplate.Replace("UniGetUI", appName);
            string expectedWingetUI = expectedWingetUITemplate.Replace("UniGetUI", appName);
            
            LanguageEngine engine = new();

            engine.LoadLanguage(language);
            Assert.Equal(expectedLog, engine.Translate("WingetUI Log"));
            Assert.Equal(expectedWingetUI, engine.Translate("WingetUI"));
        }
