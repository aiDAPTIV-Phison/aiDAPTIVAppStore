...
internal sealed class ScoopPkgOperationHelper : BasePkgOperationHelper
{
        ...
        InstallOptions options, OperationType operation)
    {
        ...
        {
            // Clear download cache after successful install.
            parameters.Add("-k");
        ...
        int returnCode)
    {
        ...
        // For install/update, use post-install verification as final verdict.
        // This avoids false negatives when cleanup emits errors but final state is correct.
        if (operation is OperationType.Install or OperationType.Update)
        {
            bool installed = IsPackageInstalled(package);
            if (installed)
            {
                if (returnCode is not 0)
                {
                    Logger.Warn($"Scoop returned non-zero exit code ({returnCode}) for {package.Id}, but post-install verification succeeded.");
                }
                return OperationVeredict.Success;
            }

            // Verification failed: treat as failure even if returnCode is 0.
    ...
    private bool IsPackageInstalled(IPackage package)
    {
        try
        {
            ...
            return installedPackages.Any(p => p.IsEquivalentTo(package));
        }
        catch (Exception ex)
        {
            Logger.Warn($"Post-install verification failed for {package.Id}: {ex.Message}");
            return false;
        }
    }
}
